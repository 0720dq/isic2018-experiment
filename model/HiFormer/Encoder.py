...
class SwinTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, embed_dim=96,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),  # 96
                               input_resolution=(patches_resolution[0] // (2 ** i_layer),  # (56,56)
                                                 patches_resolution[1] // (2 ** i_layer)),
                               depth=depths[i_layer],  # 2
                               num_heads=num_heads[i_layer],  # 3
                               window_size=window_size,  # 7
                               mlp_ratio=self.mlp_ratio,  # 4.
                               qkv_bias=qkv_bias, qk_scale=qk_scale,  # qkv_bias=True, qk_scale=None
                               drop=drop_rate, attn_drop=attn_drop_rate,  # 0.
                               drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                               norm_layer=norm_layer,  # [0,0.111111]
                               downsample=None)
...
class PyramidFeatures(nn.Module):
    def __init__(self, config, img_size=224, in_channels=3):
        ...
        resnet = config.cnn_backbone(weights=config.resnet_weights)
    ...
    def forward(self, x):
        ...
        for i in range(5):
            ...
            # Level 1
        fm1 = x  # torch.Size([16, 64, 56, 56])
        fm1_ch = self.p1_ch(x)  # torch.Size([16, 96, 56, 56])
        fm1_reshaped = Rearrange('b c h w -> b (h w) c')(fm1_ch)  # torch.Size([16, 3136, 96])
...
class All2Cross(nn.Module):
    def __init__(self, config, img_size=224, in_chans=3, embed_dim=(96, 384), norm_layer=nn.LayerNorm):
        ...
        total_depth = sum([sum(x[-2:]) for x in config.depth])  # 1
        ...
        for idx, block_config in enumerate(config.depth):
            # print(block_config)  # [1, 1, 0]
            curr_depth = max(block_config[:-1]) + block_config[-1]  # 1
            dpr_ = dpr[dpr_ptr:dpr_ptr + curr_depth]  # [0.0]
...
def main():
    ...
    model = All2Cross(config=config)
    # Py=PyramidFeatures(config=config)
    # print(Py.resnet_layers)

    img = torch.randn(16, 3, 224, 224)
    # img = torch.randn(16, 3, 512, 512)

    xs = model(img)

    for item in xs:  # 直接遍历
        print(item.shape)
    # torch.Size([16, 3137, 96])
    # torch.Size([16, 197, 384])
