...
class Attention(torch.nn.Module):
    def __init__(self, dim, img_shape, att_shape, key_dim=32, num_heads=4, attn_ratio=2, activation=torch.nn.Hardswish):
        super().__init__()
...
class CrossScaleAttention(nn.Module):
    # def __init__(self, dim, img_shape=4096, att_shape=5604):
    def __init__(self, dim, img_shape=1024, att_shape=1401):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # attn = attn.permute(0, 2, 1).contiguous().view(x0.shape[0], x0.shape[1], 64, 64)
        attn = attn.permute(0, 2, 1).contiguous().view(x0.shape[0], x0.shape[1], 32, 32)
...
class CSI(nn.Module):
    def __init__(self, dim, num_layers=1, channels=[64, 128, 256, 512], downsample=1):
        super().__init__()
        ...
        self.block = nn.ModuleList([
            CSIBlock(self.hidden_dim, channels)
...
if __name__ == '__main__':
    model = CSI(960)
    # x1 = torch.randn(2, 64, 512, 512)
    # x2 = torch.randn(2, 128, 256, 256)
    # x3 = torch.randn(2, 256, 128, 128)
    # x4 = torch.randn(2, 512, 64, 64)
    x1 = torch.randn(2, 64, 256, 256)
    x2 = torch.randn(2, 128, 128, 128)
    x3 = torch.randn(2, 256, 64, 64)
    x4 = torch.randn(2, 512, 32, 32)
    x = tuple([x1, x2, x3, x4])
    y = model(x)
