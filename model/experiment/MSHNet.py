...
if __name__ == '__main__':
    ...
    label = torch.randn(4, 1, 512, 512)
    input_tensor = input_tensor.to(device, dtype=torch.float)
    label = label.to(device, dtype=torch.float)

    masks, pred = model(input_tensor)
    # 使用 enumerate 打印索引和元素
    for index, element in enumerate(masks):
    ...
    loss = loss + loss_fun(pred, label, 5, 100)
    for j in range(len(masks)):
        print(j)
        if j > 0:
            label = down(label)
        loss = loss + loss_fun(masks[j], label, 5, 100)
