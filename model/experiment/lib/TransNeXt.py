...
class TransNeXt(nn.Module):
    ...
    def __init__(self, img_size=224, pretrain_size=None, window_size=[3, 3, 3, None],
        ...
        self.decoder3 = shiftedBlock(64, 32, False)
        self.decoder2 = shiftedBlock(32, 16, False)
        self.decoder1 = shiftedBlock(16, 8, False)
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            x, H, W = patch_embed(x)
            print('x_patch_embed1.shape=', x.shape)
            ...
            print('x_patch_embed2.shape=', x.shape)
            x = norm(x)
    ...
    def forward(self, x):
        ...
        print('x[0].shape=', x[2].shape)
        print('x[0].shape=', x[3].shape)
        # x = self.head(x)
        x4 = self.up1(x[3])
        x4 = self.conv1(x4)
        x4 = self.bn(x4)
        x4 = self.active(x4)
        x3 = self.decoder3(x4 + x[2])
        x2 = self.decoder2(x3 + x[1])
        x1 = self.decoder1(x2 + x[0])
        x_out = self.up2(x1)
...
class transnext_tiny(TransNeXt):
    def __init__(self):
        super().__init__(window_size=[3, 3, 3, None],
                         patch_size=4, embed_dims=[16, 32, 64, 128], num_heads=[2, 8, 16, 32],
...
if __name__ == '__main__':
    # stu_info = {'pretrained': None, 'img_size': 512, 'pretrain_size': None}
    # model = transnext_tiny(**stu_info)
    ...
    # 使用 enumerate 打印索引和元素
    # for index, element in enumerate(out):
    #     print(f"Index: {index}, Element.shape: {element.shape}")
    print(out.shape)