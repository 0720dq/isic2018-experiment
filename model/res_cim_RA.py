...
class Resnet34_cim_RA(nn.Module):
    def __init__(self, out_channel=1, weights=None, channel=32):
        super(Resnet34_cim_RA, self).__init__()
    ...
    def forward(self, x):
        ...
        # decoder_1 = self.final_layer(decoder_1)  # [b, 1, 128, 128]
        lateral_map_1 = F.interpolate(decoder_1, scale_factor=8, mode='bilinear')  # [b, 1, 512, 512]

        # ------------------- atten-one -----------------------
        decoder_2 = F.interpolate(decoder_1, scale_factor=0.25, mode='bilinear')  # bs, 1, 32, 32

        # cfp_out_1 = self.CFP_3(x4_rfb)  # 32 - 32  /bs, 32, 16, 16
        cfp_out_1 = x4_rfb

        decoder_2_ra = -1 * (torch.sigmoid(decoder_2)) + 1  # bs, 1, 32, 32
        aa_atten_3 = self.aa_kernel_3(cfp_out_1)  # b, 32, 32, 32
        aa_atten_3_o = decoder_2_ra.expand(-1, 32, -1, -1).mul(aa_atten_3)  # b, 32, 32, 32  /.mul函数为元素点乘

        ra_3 = self.ra3_conv1(aa_atten_3_o)  # b, 32, 32, 32
        ra_3 = self.ra3_conv2(ra_3)  # b, 32, 32, 32
        ra_3 = self.ra3_conv3(ra_3)  # 32 - 1  /bs, 1, 32, 32

        x_3 = ra_3 + decoder_2  # bs, 1, 32, 32
        lateral_map_2 = F.interpolate(x_3, scale_factor=32, mode='bilinear')  # bs, 1, 512, 512

        # ------------------- atten-two -----------------------
        decoder_3 = F.interpolate(x_3, scale_factor=2, mode='bilinear')  # bs, 1, 64, 64

        # cfp_out_2 = self.CFP_2(x3_rfb)  # 32 - 32  /bs, 32, 32, 32
        cfp_out_2 = x3_rfb

        decoder_3_ra = -1 * (torch.sigmoid(decoder_3)) + 1  # bs, 1, 64, 64
        aa_atten_2 = self.aa_kernel_2(cfp_out_2)  # bs, 32, 64, 64
        aa_atten_2_o = decoder_3_ra.expand(-1, 32, -1, -1).mul(aa_atten_2)  # bs, 32, 64, 64

        ra_2 = self.ra2_conv1(aa_atten_2_o)  # bs, 32, 64, 64
        ra_2 = self.ra2_conv2(ra_2)  # bs, 32, 64, 64
        ra_2 = self.ra2_conv3(ra_2)  # 32 - 1  /bs, 1, 64, 64

        x_2 = ra_2 + decoder_3  # bs, 1, 64, 64
        lateral_map_3 = F.interpolate(x_2, scale_factor=16, mode='bilinear')  # bs, 1, 512, 512

        # ------------------- atten-three -----------------------
        decoder_4 = F.interpolate(x_2, scale_factor=2, mode='bilinear')  # bs, 1, 128, 128

        # cfp_out_3 = self.CFP_1(x2_rfb)  # 32 - 32  /bs, 32, 64, 64
        cfp_out_3 = x2_rfb

        decoder_4_ra = -1 * (torch.sigmoid(decoder_4)) + 1  # bs, 1, 128, 128
        aa_atten_1 = self.aa_kernel_1(cfp_out_3)  # bs, 32, 128, 128
        aa_atten_1_o = decoder_4_ra.expand(-1, 32, -1, -1).mul(aa_atten_1)  # bs, 32, 128, 128

        ra_1 = self.ra1_conv1(aa_atten_1_o)  # bs, 32, 128, 128
        ra_1 = self.ra1_conv2(ra_1)  # bs, 32, 128, 128
        ra_1 = self.ra1_conv3(ra_1)  # 32 - 1  /bs, 1, 128, 128

        x_1 = ra_1 + decoder_4  # bs, 1, 128, 128
